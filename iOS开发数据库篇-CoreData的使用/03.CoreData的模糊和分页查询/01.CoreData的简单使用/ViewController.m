...
- (IBAction)addEmployee {
    ...
    for (int i = 0; i < 15; i++) {
        ...
        emp.name = [NSString stringWithFormat:@"张三%d", i];
        emp.birthday = [NSDate date];
        emp.height = 1.60; // [@(1.80 + i) floatValue];
    }
...
- (IBAction)readEmployee {
    ...
    // 2.设置过滤条件
    // 条件1：BEGINSWITH 名字以"张三1"开头
//    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"name BEGINSWITH %@", @"张三1"];
//    employeeRequest.predicate = predicate;
    
    // 条件2：ENDSWITH 名字以"1"结尾
//    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"name ENDSWITH %@", @"1"];
//    employeeRequest.predicate = predicate;
    
    // 条件3：CONTAINS 名字包含"三1"
//    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"name CONTAINS %@", @"三1"];
//    employeeRequest.predicate = predicate;

    // 条件4：like 名字以"*三12"
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"name like %@", @"*三12"];
    employeeRequest.predicate = predicate;
